...
class DonetoVerifiedSearchScreen extends StatelessWidget {
  const DonetoVerifiedSearchScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<HomeBloc, HomeState>(
      builder: (context, state) {
        final fullList = state.verifiedFundraisers;
        final searchQuery = state.query;
        
        final results = searchQuery.isEmpty
            ? fullList
            : fullList.where((f) {
                final l = f.title.toLowerCase();
                final d = f.description.toLowerCase();
                final q = searchQuery.toLowerCase();
                return l.contains(q) || d.contains(q);
              }).toList();

        if (results.isEmpty) {
          return Center(
            child: Text(
              'No matching fundraisers found',
